...
public class ProductState implements Serializable {
	...
	public ProductState copy() {
		PhysicalProperty newLocation = null;
		PhysicalProperty newProcessComplete = null;
		
		if (this.location !=null) {
			newLocation = this.location.copy();
		}
		if (this.processCompleted !=null) {
			newProcessComplete = this.processCompleted.copy();
		}
		
		return new ProductState(this.holdingObject, newProcessComplete,newLocation);
	}
}